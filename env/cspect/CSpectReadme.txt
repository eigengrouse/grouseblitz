#CSpect v3.1.0.0 ZXSpectrum emulator by Mike Dailly
(c)Copyright 1998-2026 All rights reserved

Be aware...this emulator is far from well tested, and might crash for any reason - sometimes just out of pure spite!

NOTE: DISTRIBUTION IS NOT PERMITTED WITHOUT WRITTEN CONSENT.

#CSpect can be found at: https://mdf200.itch.io/cspect
...
v3.1.0.0
---------
Added .FW support with -fw on the command line. Will load from the SD card.
Fixed up SD Card reading when there is no MBR
You can now CTRL+V (paste) into the debug text window
Added PWRITE debugger command, for a break on write to physical memory (0-2mb)
PLUGIN API: PeekPhysical/PeekPhysicalULA now takes optional buffer for filling (saving reallocation/freeing)
PLUGIN API: Peek multiple now takes optional buffer for filling (saving reallocation/freeing)
PLUGIN API: Can now ask what PC address wrote to any address
New Memory Viewer Plugin to view memory as bitmaps "<ctrl><alt><d>"
Physical and Logical breakpoints now available in 16bit mode
You can now point the disassembly window anywhere in memory.
 + < 65536, memory will point to z80 memory as normal
 + >=65536, memory will point to the bank:offset
 + ULA enabling is also used when in 24bit mode so you can view under the ULA (layer 2 mapping allows the cpu to run there)
 + Labels will goto the 24bit location, unless it's currently banked in, in which case it'll go there.
 + Labels are displayed in upper memory where 24bit labels are available
 + You can use F9 to drop breakpoints in 24bit memory. Shift-F9 has no effect in upper memory
